...
<html>
<head>
  <title>Web Semantik - Mini Project</title>
...
<?php
    ...
    $uri_rdf = 'http://localhost/webs/satriani_prj.rdf';
    ...
    foreach ($doc->all('owl:sameAs') as $akun) {
        $joe_uri = $akun->get('foaf:homepage');
        break;
    }
    ...
    */
    ...
    $sparql_query = '
      SELECT distinct * WHERE {
           <' . $joe_uri . '> dbo:birthPlace ?tempat_lahir ;
               rdfs:comment ?info ;
               dbp:instrument ?instrumen ;
               foaf:isPrimaryTopicOf ?wiki .
           ?tempat_lahir rdfs:label ?tempat_lahir_label ;
...
<div class="container">
      <div class="header clearfix">
        ...
        <h3 class="text-muted">Web Semantik Mini Project (Sample)</h3>
      </div>
      ...
      <div class="row">
        ...
        <div class="col-lg-8">
          <h4>Nama Depan: <?= $doc->get('foaf:givenName') ?></h4>
          <h4>Nama Belakang: <?= $doc->get('foaf:familyName') ?></h4>
          <h4>Instrumen: <?= $detail['instrumen'] ?></h4>
          <br>
          <h4>Tempat Lahir: <?= $detail['tempat_lahir'] ?></h4>
